...
var CGameLogic = cc.Class.extend(
    {
        m_cbCardListData:
            [
...
//			 if ( my_three[i-1][0].value != my_three[i][0].value )
        ...
        getDoubleByDouble:function(data, s_data, is_add_king, bool)
        {
            ...
            //lm.log("LIANDUI------------------------- "+JSON.stringify(newGroupData)+"  "+s_data.length);
            return newGroupData;
        },
        /**
         * 获取顺子
        ...
        getStright:function(data, s_data, is_add_king, bool)
        {
            ...
            lm.log("shunzi--------newGroupData===  "+JSON.stringify(newGroupData));
            if ( bool ) {return newGroupData;}
            if ( is_add_king )
            {
                var boom = this.getBoom(data, s_data, is_add_king);
        ...
        getRandomPuker:function(intelligentData)
        {
            ...
            cc.log("randomdata=  "+JSON.stringify(data));
            ...
            if ( rData01.length > 0 )
            {
                ...
                lm.log("rData01.slice(0, 1).length= "+rData01.slice(0, 1)[0].length);
            }
            if ( rData02.length > 0 )
            {
                temp.push(rData02.slice(0, 1));
                lm.log("rData02.slice(0, 1).length= "+rData02.slice(0, 1)[0].length);
            }
            if ( rData03.length > 0 )
            {
                temp.push(rData03.slice(0, 1));
                lm.log("rData03.slice(0, 1).length= "+rData03.slice(0, 1)[0].length);
            }
            if ( rData04.length > 0 )
            {
                temp.push(rData04.slice(0, 1));
                lm.log("rData04.slice(0, 1).length= "+rData04.slice(0, 1)[0].length);
            }
            if ( rData05.length > 0 )
            {
                temp.push(rData05.slice(0, 1));
                lm.log("rData05.slice(0, 1).length= "+rData05.slice(0, 1)[0].length);
            }
            if ( rData06.length > 0 )
            {
                temp.push(rData06.slice(0, 1));
                lm.log("rData06.slice(0, 1).length= "+rData06.slice(0, 1)[0].length);
            }
            ...
            if ( rData07.length > 0 )
            {
                ...
                lm.log("rData07.slice(0, 1).length= "+rData07.slice(0, 1)[0].length);
            }
            if ( rData08.length > 0 )
            {
                temp.push(rData08.slice(0, 1));
                lm.log("rData08.slice(0, 1).length= "+rData08.slice(0, 1)[0].length);
            }
            if ( rData09.length > 0 )
            {
                temp.push(rData09.slice(0, 1));
                lm.log("rData09.slice(0, 1).length= "+rData09.slice(0, 1)[0].length);
            }
            if ( rData010.length > 0 )
            {
                temp.push(rData010.slice(0, 1));
                lm.log("rData010.slice(0, 1).length= "+rData010.slice(0, 1)[0].length);
            }
            if ( rData011.length > 0 )
            {
                temp.push(rData011.slice(0, 1));
                lm.log("rData011.slice(0, 1).length= "+rData011.slice(0, 1)[0].length);
            }
            for ( var i = 0; i < temp.length; i++ )
            {
                lm.log("temp[i].length= "+temp[i][0].length);
            }
            return temp;
        },
        /**
        ...
        getRandomPukerHuadong:function(currentPuker)
        {
            ...
            lm.log("currentdpukerdata======== "+JSON.stringify(currentPuker));
            ...
            lm.log("temp======== "+JSON.stringify(temp));
            if ( temp.length < 1 )
            {
                return [[currentPuker]];
            }
        ...
        getAllSunzi:function(type)
        {
            ...
            //}();
            lm.log("getAllSunzi-------------- "+JSON.stringify(rData04));
            return rData04;
        },
        //获取说有连对
        getAllDoubleSun:function(type)
        {
            ...
            //}();
            lm.log("getAllDoubleSun-------------- "+JSON.stringify(rData05));
        },
        //获取所有飞机
        getAllThreeByThree:function(type)
        {
            ...
            //}();
            lm.log("getAllThreeByThree-------------- "+JSON.stringify(rData06));
            return rData06;
        },
        /**
...
//var gameLogic = gameLogic || new CGameLogic();
