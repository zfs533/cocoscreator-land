...
var LandAnimate = rootLayer.extend(
	{
		...
		setVariable:function()
		{
			...
			cc.spriteFrameCache.addSpriteFrames("res/landlord/animate/animate_rocket.plist");
			cc.spriteFrameCache.addSpriteFrames("res/landlord/animate/animate_other.plist");
			cc.spriteFrameCache.addSpriteFrames("res/landlord/animate/animate_bomb.plist");
		},
		zinit:function()
		{
		...
		playRocketAnimation:function(direction, type)
		{
			...
			sp.setPosition(this.size.width/2, this.size.height/2 + 50);
			...
			for ( var i = 1; i <= 15; i++ )
			{
				...
				if ( i < 10 )
				{
					str = "ROCKET_0"+i.toString() +".png";
				}
				else
				{
					str = "ROCKET_"+i.toString() +".png";
				}
			...
			var sequnce = cc.sequence(animate,cc.removeSelf());
		...
		playBomeAnimation:function(direction, type)
		{
			...
			this.aLayer.removeAllChildren(true);
			var sp = cc.Sprite.create();
			sp.setPosition(this.size.width/2, this.size.height/2+180);
			this.aLayer.addChild(sp, 100);
			var animaFrames = [];
			for ( var i = 1; i <= 20; i++ )
			{
				...
				if ( i < 10 )
				{
					str = "CARTOON_BOMB_0"+i.toString() +".png";
				}
				else
				{
					str = "CARTOON_BOMB_"+i.toString() +".png";
				}
				var frame = cc.spriteFrameCache.getSpriteFrame(str);
				animaFrames.push(frame);
			}
		...
		playChunTianAnimation:function(func)
		{
			...
			var animation = cc.Animation.create(animaFrames, 0.07);
			var animate = cc.Animate.create(animation);
			var sequnce = cc.sequence(animate, func, cc.removeSelf());
			sp.runAction(sequnce);
		},
		//连对
