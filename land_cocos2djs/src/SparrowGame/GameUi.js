...
var DeskLayer = cc.Layer.extend(
	{
		...
		refreshCurrentBeishu:function()
		{
			...
			var callfunc  = cc.callFunc(this.setCurrentBeishu, this);
			var sequence  = cc.sequence(spawn,delayT, spawn02,callfunc);
			beishu.runAction(sequence);
		},
		//设置倍数
		setCurrentBeishu:function(target,beisuNumber)
		{

			//cc.log("sparrowDirector.gameData.currentCallScore01======= "+sparrowDirector.gameData.currentCallScore01);
			//var beishu = sparrowDirector.gameData.currentCallScore01*2;
			var beishu = Number(this.calledscore.getString())*2;
			if ( beisuNumber )
			{
				beishu = beisuNumber;
			}
			//cc.log("sparrowDirector.gameData.currentCallScore01======= "+sparrowDirector.gameData.currentCallScore01);
            //
			//sparrowDirector.gameData.currentCallScore01 = beishu;
            //
			//cc.log("sparrowDirector.gameData.currentCallScore01======= "+sparrowDirector.gameData.currentCallScore01);
...
var OrderLayer = cc.Layer.extend(
	{
		...
		zinit:function()
		{
			...
			this.startBtn = ccui.Button.create("desk035.png","","",ccui.Widget.PLIST_TEXTURE);
		...
		addUI:function()
		{
			...
			for ( var i = 0; i < 4; i++ )
			{
				this["btn" + i] = ccui.helper.seekWidgetByName(this.ui, "btn"+i);
				this["btn" + i].name = i;
				this["btn" + i].addTouchEventListener(this.orderEvent, this);
			}
		},
		/**
		 * 启动叫分
		...
		startCallScore:function(direction, data)
		{
			...
			if ( direction == sparrowDirector.gameData.myChairIndex )
			{
				...
				{
					this.btn0.loadTextures("desk026.png","","",ccui.Widget.PLIST_TEXTURE);
					this.btn0.setTouchEnabled(false);
				}
				else if ( data.cbCurrentScore == 2 )
				{
					this.btn0.loadTextures("desk026.png","","",ccui.Widget.PLIST_TEXTURE);
					this.btn0.setTouchEnabled(false);
					this.btn2.loadTextures("desk024.png","","",ccui.Widget.PLIST_TEXTURE);
					this.btn2.setTouchEnabled(false);
				}
		...
		refreshTextures:function(orderType)
		{
			...
			this.btn0.setTouchEnabled(true);
			this.btn1.setTouchEnabled(true);
			this.btn2.setTouchEnabled(true);
			this.btn3.setTouchEnabled(true);
			if ( this.orderType == 0 )
			{
				this.btn0.loadTextures("desk015.png","","",ccui.Widget.PLIST_TEXTURE);
				this.btn1.loadTextures("desk023.png","","",ccui.Widget.PLIST_TEXTURE);
				this.btn2.loadTextures("desk025.png","","",ccui.Widget.PLIST_TEXTURE);
				this.btn3.loadTextures("desk021.png","","",ccui.Widget.PLIST_TEXTURE);
			}
			else
			{
				...
				this.btn0.loadTextures("desk001.png","","",ccui.Widget.PLIST_TEXTURE);
				this.btn1.loadTextures("desk009.png","","",ccui.Widget.PLIST_TEXTURE);
				this.btn2.loadTextures("desk013.png","","",ccui.Widget.PLIST_TEXTURE);
				this.btn3.loadTextures("desk002.png","","",ccui.Widget.PLIST_TEXTURE);
			}
		},
		...
		setOutPukerBtnEnable:function()
		{
			...
			if ( !bool )
			{
				this.btn3.loadTextures("desk020.png","","",ccui.Widget.PLIST_TEXTURE);
				this.btn3.setTouchEnabled(false);
			}
			else
			{
				this.btn3.loadTextures("desk002.png","","",ccui.Widget.PLIST_TEXTURE);
		...
		jugementOutPuker:function(pukerData)
		{
			...
			{
				this.btn0.loadTextures("desk022.png","","",ccui.Widget.PLIST_TEXTURE);
				this.btn0.setTouchEnabled(false);
				var type_select = gameLogic.getType(data);
				if ( type_select < 1 )
				{
...
//确认界面
var ConfirmNode = cc.Layer.extend({
	ctor: function (str, target, callback, callback2) {
		this._super();
		this.parentView = ccs.load("res/SparrowGames/ui/confirmNode.json").node;
		var desc_text = ccui.helper.seekWidgetByName(this.parentView, "desc_text");
		var btn_ok = ccui.helper.seekWidgetByName(this.parentView, "btn_ok");
		var btn_cancel = ccui.helper.seekWidgetByName(this.parentView, "btn_cancel");
...
var ScoreLayer = cc.Layer.extend(
	{
		...
		{
			var texture = "res/landlord/CALL_SCORE.png", index = 0;
			...
			//var score = cc.Sprite.create(texture,cc.rect(63 * index,0,63,34));
...
var OrderLayerHappy = cc.Layer.extend(
	{
		...
		reloadShowCardBtn:function(ShowCardCount)
		{
			...
			var btn_showcard = ccui.helper.seekWidgetByName(this.layer_land_showcard, "btn_showcard");
			btn_showcard.loadTextures("btn_showcard_" + beishu + ".png", "btn_showcard_" + beishu + ".png", "", ccui.Widget.PLIST_TEXTURE);
		},
...
var ResultLayer = cc.Layer.extend(
	{
		...
		showResultAnimation01:function()
		{
			...
			var str = "res/landlord/animate/shengli.json";
			var timer = 300;
			if ( score < 0 )
			{
				str = "res/landlord/animate/shibai.json";
				timer = 320;
			...
			var animate = ccs.load(str).node;
			this.addChild(animate, 0, 12345);
			animate.setPosition(winSize.width/2, winSize.height/2+0);
			var animateAction = ccs.load(str).action;
			animateAction.gotoFrameAndPlay(0, timer+1, 0, false);
		...
		refreshTextures01:function(data)
		{
			...
			{
				var beisuNumber = sparrowDirector.gameData.currentCallScore*cbb*bc*fbc;
			...
			if ( state == ccui.Widget.TOUCH_ENDED )
			{
				...
				if (Number(GetDeviceType()) == DeviceType.ANDROID) {
					...
					webMsgManager.SendGpGetOrderData(DefultIdentifier,function(data)
					{
						...
						if(ChannelLabel == "8633" && ChannelID != 4  && ChannelID != 5)
						{
						...
						else if(ChannelID == 4 || ChannelID == 5) //hanhu #联通需先完成订单信息
						{
...
var chatText = cc.Node.extend({
	ctor: function (isFace, chatDirection, color, chatContent) {
		...
		var numPerRow = 10;
		var gap = 70;
		cc.log("--------------------------chatDirection= "+chatDirection);
		switch (chatDirection) {
			case playerDirection.DOWN:
				chatBg = cc.Scale9Sprite("pop_chat_left.png", cc.rect(40, 30, 1, 1));
				this.setPosition(200, 350);
				break;

			case playerDirection.LEFT:
				chatBg = cc.Scale9Sprite("pop_chat_left.png", cc.rect(40, 25, 1, 1));
				chatBg.setFlippedX(true);
				this.setPosition(winSize.width - 200, winSize.height / 2 + 200);
				break;
		...
		if (isFace) {
			//表情
			var face = cc.Sprite.create("res/face/" + chatContent + ".png");
			this.addChild(face, 1);
			chatBg.setContentSize(100, 80);
		} else {
			//文字
...
//宝箱界面
var GameBoxLayer = cc.Layer.extend({
	ctor: function (boxData) {
		this._super();
		var self = this;
		this.parentView = ccs.load("res/SparrowGames/ui/BoxLayer.json").node;
		var mm = JSON.stringify(boxData);
		...
		this.cardResBack = [
			"res/SparrowGames/ui/gameBoxUI/21_clearing_luckycard_r5f.png",
			"res/SparrowGames/ui/gameBoxUI/21_clearing_luckycard_r4f.png",
			"res/SparrowGames/ui/gameBoxUI/21_clearing_luckycard_r3f.png",
			"res/SparrowGames/ui/gameBoxUI/21_clearing_luckycard_r2f.png",
			"res/SparrowGames/ui/gameBoxUI/21_clearing_luckycard_r1f.png"
		];
		this.cardResFore = [
			"res/SparrowGames/ui/gameBoxUI/21_clearing_luckycard_r5b.png",
			"res/SparrowGames/ui/gameBoxUI/21_clearing_luckycard_r4b.png",
			"res/SparrowGames/ui/gameBoxUI/21_clearing_luckycard_r3b.png",
			"res/SparrowGames/ui/gameBoxUI/21_clearing_luckycard_r2b.png",
			"res/SparrowGames/ui/gameBoxUI/21_clearing_luckycard_r1b.png"
		];
		this.btn_close = ccui.helper.seekWidgetByName(this.parentView, "btn_close");
		this.btn_close.setVisible(false);
		this.btn_close.addTouchEventListener(function (sender, type) {
			if (type == ccui.Widget.TOUCH_ENDED)
			{
				if ( !sparrowDirector.boxCount && sparrowDirector.gameLayer.itemBox )
				{
					sparrowDirector.gameLayer.itemBox.removeFromParent();
				}
				this.removeFromParent();
			}
		}, this)
		...
		var retainTime = ccui.helper.seekWidgetByName(this.parentView, "retainTime");
		var text_retain = ccui.helper.seekWidgetByName(this.parentView, "text_retain");
		var itemBorder = ccui.helper.seekWidgetByName(this.parentView, "itemBorder");

		//恭喜XX获奖文本
		var text_str = text_retain.getString();
		var namesStr = "";
		for (var key in boxData["dwUserID"]) {
			if (boxData["dwUserID"][key]) {
				cc.log("has value of key " + key);
				if (namesStr != "")
					namesStr += "、";
				namesStr += sparrowDirector.getInfoOfPlayers(boxData["dwUserID"][key])["szNickName"];
				cc.log("namestring is " + namesStr);
			}
		}
		text_str = "恭喜" + namesStr + text_str;
		text_retain.setString(text_str);


		//for (var key in this.boxData["dwUserID"]) {
		//    if (this.boxData["dwUserID"][key] == userInfo.globalUserdData["dwUserID"]) {
		//        this.myBoxKey = key;
		//    }
		//}
		this.myBoxKey = this.boxData["dwChestIndex"];
		cc.log("myBoxKey is " + this.myBoxKey);

		var boxLevel = Number(boxData["cbCardLevel"] - 1);
		cc.log("boxLevel is " + boxLevel);

		this.itemsButton = [];
		for (var i = 0; i < 4; i++) {

			var btn_cardNode = ccs.load("res/SparrowGames/ui/boxItem.json").node;
			var btn_card = ccui.helper.seekWidgetByName(btn_cardNode, "btn_card");
			btn_card.loadTextures(this.cardResBack[boxLevel], this.cardResBack[boxLevel], this.cardResFore[boxLevel]);

			btn_card.item_img = ccui.helper.seekWidgetByName(btn_cardNode, "item_img");
			btn_card.itemName = ccui.helper.seekWidgetByName(btn_cardNode, "itemName");
			btn_card.label_num = ccui.helper.seekWidgetByName(btn_cardNode, "label_num");
			btn_card.label_const = ccui.helper.seekWidgetByName(btn_cardNode, "label_const");
			btn_card.itemNum = ccui.helper.seekWidgetByName(btn_cardNode, "itemNum");
			btn_card.itemConst = ccui.helper.seekWidgetByName(btn_cardNode, "itemConst");
			btn_card.getPlayerLabel = ccui.helper.seekWidgetByName(btn_cardNode, "getPlayerLabel");
			btn_card.gotPlayerBg = ccui.helper.seekWidgetByName(btn_cardNode, "gotPlayerBg");
			btn_card.infoPanel = ccui.helper.seekWidgetByName(btn_cardNode, "infoPanel");

			btn_cardNode.setPosition(i * 192, 5);
			itemBorder.addChild(btn_cardNode);

			btn_card.infoPanel.setVisible(false);
			btn_card.index = i;
			btn_card.myBoxKey = this.myBoxKey;
			this.itemsButton.push(btn_card);

			btn_card.addTouchEventListener(function (sender, type) {
				if (type == ccui.Widget.TOUCH_ENDED) {
					if (!self.mySelected) {
						sparrowDirector.SendOpenBox(sender.myBoxKey, userInfo.globalUserdData["dwUserID"], sender.index);
						self.mySelected = true;
					}
				}
			}, this);
		}
		this.m_nCardTrans = [0, 1, 2, 3];
		this.m_bOpened = [false, false, false, false];
		this.mySelected = false;

		this.retainTimeCount = 20;
		this.schedule(function (dt) {
			self.retainTimeCount--;
			cc.log("retain time is " + self.retainTimeCount);
			if (self.retainTimeCount <= 0) {
				self.retainTimeCount = 0;
				if (self.step2) {
					self.removeFromParent();
				}
				if (!self.mySelected) {
					var canSendIndex = [];
					for (var key in this.m_bOpened) {
						if (!this.m_bOpened[key])
							canSendIndex.push(key);
					}
					var selectedKey = canSendIndex[DataUtil.getRandomInt(0, canSendIndex.length - 1)];
					sparrowDirector.SendOpenBox(this.myBoxKey, userInfo.globalUserdData["dwUserID"], selectedKey);
					self.mySelected = true;
					//self.setStep2();
				}

			}
			retainTime.setString("倒计时：" + self.retainTimeCount);
		}, 1);

	},
	//第二步：10s倒计时
	setStep2: function () {
		this.retainTimeCount = 10;
		this.step2 = true;
		this.btn_close.setVisible(true);
		for (var i = 0; i < 4; i++) {
			if (!this.m_bOpened[i]) {
				this.rotateBox(this.m_nCardTrans[i], true);
			}
		}
	},
	//开宝箱
	openBox: function (hitData) {
		cc.log("hitData is " + JSON.stringify(hitData));
		//查找现在的位置
		var nIndex = -1;
		for (var i = 0; i < 4; i++) {
			if (this.boxData["dwUserID"][i] == hitData["dwUserID"]) {
				nIndex = i;
				break;
			}
		}
		if (nIndex != hitData["cbIndex"]) //判断是否需要转换
		{
			//进行转换
			this.m_nCardTrans[hitData["cbIndex"]] = nIndex;
			//查找位置
			for (var i = 0; i < 4; i++) {
				if (i != hitData["cbIndex"] && this.m_nCardTrans[i] == nIndex) {
					this.m_nCardTrans[i] = hitData["cbIndex"];
					break;
				}
			}
		}

		this.m_bOpened[nIndex] = true;
		this.rotateBox(hitData["cbIndex"]);

	},
	//翻转卡牌(第二个参数，是否自动翻转)
	rotateBox: function (index, rotatedBySys) {
		var self = this;
		var sender = this.itemsButton[index];

		var action = cc.sequence(cc.scaleTo(0.2, 0, 1), cc.callFunc(function () {
			sender.setEnabled(false);
			sender.setBright(false);
		}, this), cc.scaleTo(0.2, 1, 1), cc.callFunc(function () {
			sender.infoPanel.setVisible(true);
			//hanhu #显示奖品图片 2015/08/14
			//console.log("宝箱图片ID为：" + this.boxData["dwIcon"][this.m_nCardTrans[index]]);
			sender.item_img.loadTexture("res/cocosOut/product/" + this.boxData["dwIcon"][this.m_nCardTrans[index]] + ".jpg" );
			sender.itemName.setString(this.boxData["szName"][this.m_nCardTrans[index]]);
			sender.itemNum.setString(this.boxData["lPresentValue"][this.m_nCardTrans[index]]);
			sender.itemConst.setString(Number(this.boxData["dwPrice"][this.m_nCardTrans[index]])/100 + "元");
			//console.log("玩家id为:" + this.boxData["dwUserID"][this.m_nCardTrans[index]]);
			var userID = this.boxData["dwUserID"][this.m_nCardTrans[index]];
			if(userID != 0)  //hanhu #若玩家ID为0则不设置 2015/08/14
			{
				sender.getPlayerLabel.setString(sparrowDirector.getInfoOfPlayers(this.boxData["dwUserID"][this.m_nCardTrans[index]])["szNickName"]);
			}else
			{
				sender.getPlayerLabel.setString("");
			}
			self.bAllOpen = true;
			for (var i = 0; i < 4 && self.bAllOpen; i++) {
				if (self.boxData["dwUserID"][i] && !self.m_bOpened[i])
					self.bAllOpen = false;
			}
			if (self.bAllOpen && !rotatedBySys)//全开了
			{
				self.setStep2();
			}
		}, this));
...
var popUpTalkDialog = cc.Layer.extend({
	...
	ctor: function () {
		this._super();
		this.parentView = ccs.load("res/SparrowGames/ui/talkTextUI.json").node;
		this.addChild(this.parentView);
		this.parentView.setPosition((winSize.width - 960) / 2, -200);
		this.isRoomChart = true;

		var text_target = ccui.helper.seekWidgetByName(this.parentView, "text_target");
		var btn_targetSelect = ccui.helper.seekWidgetByName(this.parentView, "btn_targetSelect");
		var talkContent = ccui.helper.seekWidgetByName(this.parentView, "talkContent");
		var btn_face = ccui.helper.seekWidgetByName(this.parentView, "btn_face");
		var targetSelector = ccui.helper.seekWidgetByName(this.parentView, "targetSelector");
		var btn_chatW = ccui.helper.seekWidgetByName(this.parentView, "btn_chatW");
		var btn_chatR = ccui.helper.seekWidgetByName(this.parentView, "btn_chatR");
		var btn_send = ccui.helper.seekWidgetByName(this.parentView, "btn_send");
		this.listPanel = ccui.helper.seekWidgetByName(this.parentView, "listPanel");
		this.faceList = ccui.helper.seekWidgetByName(this.parentView, "faceList");

		talkContent.addEventListener(function(sener, type){
			if (type == ccui.TextField.EVENT_ATTACH_WITH_IME)
			{
				this.isWrite = true;
				this.y = winSize.height/3;
			}

		},this);

		//hanhu #解决键盘无法关闭的问题
		btn_send.setSwallowTouches(false);
		//add faces to listview
		for (var i = 0; i < this.rowNumber; i++) {
			this.pushFaceLine();
		}
		this.listPanel.setVisible(false);

		btn_face.addTouchEventListener(function (sender, type) {
			if (type == ccui.Widget.TOUCH_ENDED) {
				this.listPanel.setVisible(true);
			}
		}, this);

		btn_targetSelect.addTouchEventListener(function (sender, type) {
			if (type == ccui.Widget.TOUCH_ENDED) {
				targetSelector.setVisible(true);
			}
		}, this);

		btn_chatW.addTouchEventListener(function (sender, type) {
			if (type == ccui.Widget.TOUCH_ENDED) {
				text_target.setString("喇叭喊话");
				this.isRoomChart = false;
				this.chartType = 1;
				targetSelector.setVisible(false);
			}
		}, this);

		btn_chatR.addTouchEventListener(function (sender, type) {
			if (type == ccui.Widget.TOUCH_ENDED) {
				text_target.setString("房间聊天");
				this.isRoomChart = true;
				this.chartType = 0;
				targetSelector.setVisible(false);
			}
		...
		btn_send.addTouchEventListener(function (sender, type) {
			...
			{
				//关闭输入键盘
				cc.director.getOpenGLView().setIMEKeyboardState(false);
				//if (this.isRoomChart)
				{
					//空消息禁止发送
					if(talkContent.getString() != "")
					{
						sparrowDirector.SendUserChat(this.chartType, 0, talkContent.getString() + " ");
					}
	...
	//加载表情
	pushFaceLine: function () {
		var layout = ccui.Layout.create();
		layout.setContentSize(cc.size(800, 50));
		for (var i = 0; i < this.numPerRow; i++) {
			var faceNumber = this.numPerRow * this.faceList.getItems().length + i;
			var btn = ccui.Button.create("res/face/" + faceNumber + ".png", "res/face/" + faceNumber + ".png", "res/face/" + faceNumber + ".png", 0);
			if (btn) {
				btn.faceNumber = faceNumber;
				btn.setPosition(i * 50, 0);
				btn.setAnchorPoint(0, 0);
				layout.addChild(btn);
				btn.addTouchEventListener(this.faceCallBack, this);
			}
		}
		this.faceList.pushBackCustomItem(layout);
	},
	//表情回调
	faceCallBack: function (sender, type) {
		if (type == ccui.Widget.TOUCH_ENDED) {
			cc.director.getOpenGLView().setIMEKeyboardState(false);
			if(this.chartType == 1)
			{
				layerManager.PopTipLayer(new PopAutoTipsUILayer("抱歉，表情无法在喇叭中使用", 5),true);
			}
			else
			{
				sparrowDirector.SendUserExpression(0, sender.faceNumber);
				this.removeFromParent();
			}
		}
	},
...
var userInfoPopup = cc.Node.extend({
	ctor: function (data) {
		...
		{
			lm.log("yyp 3+  " + wFaceID + " " + wCustomFaceID + " " + wUserID + " ");
			//系统头像
			this.CustomFaceLayer.SetVisable(false);
