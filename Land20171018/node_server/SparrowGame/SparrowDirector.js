...
var SparrowDirector = BaseDirector.extend(
	{
		...
		happydoudizuListeners:function()
		{
			...
			//连胜信息
			connectUtil.dataListenerManualEx(LandGameID.DOU_DI_ZHU_HAPPY,KernelCurrent, LandCrazyExMainID, LandCrazyExGameMsgHappy.SUB_S_CONTINUE_WIN,function(ServerlizeObject, wDataSize)
			{
				var data = {};
				data.type = "CMD_S_continue_win";
				//连胜局数
				data.wContinueWinCount = DataUtil.ReadNumber(ServerlizeObject, 16);
				//连胜获奖奖品类型
				data.cbPrizeType = DataUtil.ReadNumber(ServerlizeObject, 8);
				//奖品数量
				data.lPrizeCount = DataUtil.ReadNumber(ServerlizeObject, 64);

				lm.log("欢乐斗地主 -> 连胜信息 CMD_S_continue_win -> [ " + JSON.stringify(data) + "]");
				self.continueWinData = data;
			});
			...
			connectUtil.dataListenerManualEx(LandGameID.DOU_DI_ZHU_HAPPY,KernelCurrent, LandCrazyExMainID, LandCrazyExGameMsgHappy.SUB_S_GAME_CONCLUDE,function(ServerlizeObject, wDataSize)
			{
				...
				//是否完成任务
				data.bFinishTask = DataUtil.ReadNumber(ServerlizeObject, 8);
				//任务加倍
				data.cbTaskAddTimes = DataUtil.ReadNumber(ServerlizeObject, 8);
		...
		doudizuListeners:function()
		{
			...
			//连胜信息
			connectUtil.dataListenerManualEx(LandGameID.DOU_DI_ZHU,KernelCurrent, LandCrazyExMainID, LandCrazyExGameMsg.SUB_S_CONTINUE_WIN,function(ServerlizeObject, wDataSize)
			{
				var data = {};
				data.type = "CMD_S_continue_win";
				//连胜局数
		...
		castData:function(data)
		{
			...
			switch (data.type)
			{
				...
				case "CMD_S_GameConclude":
				{
					if(data.bFinishTask)
					{
						sparrowDirector.gameLayer.deskLayer.refreshCurrentBeishu(data.cbTaskAddTimes);
						sparrowDirector.taskTimes = data.cbTaskAddTimes;
					}
					this.overData = data;
					if(!sparrowDirector.isPlayingAnimate)
						this.gameFinshfunc();
		...
		gameFinshfunc:function()
		{
			var self = this;
			if(Is_HAPPY_ROOM)
			{
				//春天 反春
				if(self.overData.bChunTiam == 1 || self.overData.bFanChunTian)
				{
					var self = this;
					sparrowDirector.gameLayer.landAnimate.playChunTianAnimation(cc.callFunc(function()
					{
